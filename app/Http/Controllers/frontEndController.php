...
class frontEndController extends Controller{
  ...
  public function allCategoryWithItem ($id) {
  ...
  public function index($id){
    ...
    $selected_cat_id = $this->allCategoryWithItem($id);
  ...
  public function welcome(){
    ...
    $data = DB::select("select settings.title, settings.address , settings.logo, settings.shortcode, settings.lat , settings.lon , users.name , users.id from users , settings where settings.user_id = users.id");
    $meta = [];
  ...
  public function get_next_element ($needle, $haystack) {
  ...
  public function get_prev_elemnt($needle, $haystack) {
    if (count($haystack) < 2) {
      return false;
    }
    $index = array_search($needle, $haystack);
  ...
  public function get_next_category($id, $cat) {
    $catID = Categories::where('slug', $cat)->pluck('id')->first();
    $selected_cat_id = $this->allCategoryWithItem($id);
    // return in_array($catID, $selected_cat_id);
    $element = $this->get_next_element($catID, $selected_cat_id);
    if ($element) {
  ...
  public function get_prev_category($id, $cat) {
    $catID = Categories::where('slug', $cat)->pluck('id')->first();
    $selected_cat_id = $this->allCategoryWithItem($id);
    $element = $this->get_prev_elemnt($catID, $selected_cat_id);
    if ($element) {
      return Categories::find($element);
    }else {
      return false;
  ...
  public function menu($id, $cat = 0){
    ...
    $prev_category = $this->get_prev_category($id, $cat);
    $next_category = $this->get_next_category($id, $cat);
    $id = request()->route('id');
        $cat_name = $cat;
    ...
    $selectedCategory = Categories::where('user_id', $data->id)->where('slug', $cat_name)->first();
    $surveys = Survey::where('user_id', $data->id)->approved()->get();

    return view(
      'front.menu',
      compact('data', 'dishes', 'drinks','specialMenu', 'setting', 'cat', 'id', 'user_id', 'surveys', 'selectedCategory', 'prev_category', 'next_category')
    );
  }
  ...
  public function catmenu($id, $cat){
    ...
    return view(
      'front.page',
      compact('data', 'setting', 'page', 'categories', 'cat', 'user_id', 'surveys', 'cat_name', 'prev_category', 'next_category')
    );
  }
