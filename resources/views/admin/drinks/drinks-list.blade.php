    ...
    <div class="padding">
      <div class="box">
        <div class="box-header">
          ...
          ?>
          <h2 style="display: inline-block ; text-transform: capitalize"><?= $modleName ?></h2> | <a href="{{ action('admin\DrinksController@create') }}" class='md-btn md-raised m-b-sm w-xs green' style="font-size: 0.91em;margin-bottom: -7px !important;margin-left: 15px;max-width: 80px;font-size: 11px;">Add Product</a>
        </div>
  ...
  <script>
			$(function () {
					$('.legecyChanger').on("change" , function (e) {
							...
							$.ajax({
									url : '<?= action('admin\DrinksController@drinksPoster') ?>' ,
									type : 'post' ,
									data : {
