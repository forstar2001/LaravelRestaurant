    ...
    <div class="padding">
      <div class="box">
        <div class="box-header">
          ...
          ?>
          <h2 style="display: inline-block ; text-transform: capitalize"><?= $modleName ?></h2> | <a href="{{ action('admin\DishesController@create') }}" class='md-btn md-raised m-b-sm w-xs green' style="font-size: 0.91em;margin-bottom: -7px !important;margin-left: 15px;max-width: 80px;font-size: 11px;">Add Product</a>
        </div>
        <div class="table-responsive">
          <table class="table table-striped b-t b-b" id="tblData">
            ...
            <tr data-row-id="<?= $row[ 'id' ] ?>">
              ...
              <td>
                <a href="<?=  action('admin\DishesController@edit', ['id' => $row[ 'id' ]]) ?>" class="btn btn-fw success pull-left">Edit</a>
                <form class="pull-right" role="form" method="post" style="float: left;
    margin-left: 10px;" action="<?= action('admin\DishesController@destroy', ['id' => $row[ 'id' ]]) ?>">
                  @csrf
  ...
  <script>
			$(function () {
					$('.legecyChanger').on("change" , function (e) {
							...
							$.ajax({
									url : '<?= action('admin\DishesController@dishesPoster') ?>' ,
									type : 'post' ,
									data : {
