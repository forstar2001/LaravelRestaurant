      ...
      <div class="center-block w-xxl w-auto-xs p-y-md">
        ...
        <div class="p-a-md box-color r box-shadow-z1 text-color m-a">
          <div class="m-b text-sm">
            Sign Up for {{ base64_decode(env('s_name')) }} Account
          </div>
          <form method="POST" action="{{ route('register') }}">
            @csrf
            <div class="md-form-group float-label">
              <input id="name" type="text" class="md-input {{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ old('name') }}" required autofocus>
              @if ($errors->has('name'))
        ...
        <div class="p-v-lg text-center">
          <div class="m-b"><a ui-sref="access.forgot-password" href="{{ route('login') }}" class="text-primary _600">Sign In</a></div>
        </div>
      </div>
      <!-- ############ LAYOUT END-->
    </div>
