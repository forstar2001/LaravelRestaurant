...
<div class="app" id="app">
  ...
  <div class="center-block w-xxl w-auto-xs p-y-md">
    ...
    <div class="p-a-md box-color r box-shadow-z1 text-color m-a">
      <div class="m-b " style="display: none">
        Forgot your password?
        <p class="text-xs m-t">Enter your Email and new Password to reset Password</p>
      </div>
      <form method="POST" action="{{ route('password.request') }}">
        <div class="md-form-group">
          @csrf
          <input type="hidden" name="token" value="{{ $token }}">
          <input id="email" type="email" class="md-input {{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ $email or old('email') }}" required autofocus>
          @if ($errors->has('email'))
        ...
        <div class="md-form-group">
          <input id="password" type="password" class="md-input {{ $errors->has('password') ? ' is-invalid' : '' }}" name="password" required>
        ...
        <div class="md-form-group">
          <input id="password-confirm" type="password" class="md-input {{ $errors->has('password_confirmation') ? ' is-invalid' : '' }}" name="password_confirmation" required>
  ...
  <div class="app-footer">
    <div class="p-2 text-xs">
      <div class="pull-right text-muted py-1">
        &copy; Copyright <strong>{{ htmlentities(base64_decode(env('s_c_link'))) }}</strong> <span class="hidden-xs-down">{{ base64_decode(env('s_name')) }}</span>
